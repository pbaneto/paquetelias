      ...
      <div className="max-w-md w-full space-y-8">
        ...
        <form className="mt-8 space-y-6" onSubmit={handleSubmit}>
          <div className="rounded-md shadow-sm -space-y-px">
            ...
            {/* Only show password field in sign-in mode */}
            {!isRecoveryMode && (
              <div>
          ...
          {/* Show success message when recovery email is sent */}
          {isRecoveryMode && recoveryStatus === 'success' && (
          ...
          <div className="flex flex-col space-y-4">
            ...
            <button
              type="button" // Changed to button type to handle both cases
              disabled={isLoading}
              className="group relative w-full flex justify-center py-2 px-4 border border-transparent text-sm font-medium rounded-md text-white bg-blue-600 hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 disabled:opacity-50"
              onClick={isRecoveryMode ? handlePasswordRecovery : handleSubmit}
            >
              {isLoading
                ? (isRecoveryMode ? 'Sending...' : 'Signing in...')
                : (isRecoveryMode ? 'Recover Password' : 'Sign in')}
            </button>

            {/* Toggle between sign-in and recovery mode */}
            ...
            {/* Only show these in sign-in mode */}
            {!isRecoveryMode && (
              <>
